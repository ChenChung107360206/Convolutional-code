...
max_frame_error_count = 50;

msg_len = 100; % input msg's number
decision_type = 2; % 1 is hard-decision, 2 is soft-decision
window_len = 10;
truncation_type = 1; % 1 = fixed state, 2 = best state, 3 = majority-vote

BER_picture = zeros(3,8);
for EbN0dB = 1:5
    EbN0 = 10^(EbN0dB/10);
    sigma = sqrt( 1/(2*coderate*EbN0) );
    for truncation_type = 1:3

        frame = 0;
        frame_error_count = 0; bit_error_count = 0;
        while frame_error_count < max_frame_error_count
            frame = frame + 1;
%----------------------------------------start----------------------------------------

            %----------encode----------
            %input_msg = input("Input message : ");
            input_msg = randi([0 1], 1, msg_len); msg = input_msg; % generate message
            ...
            %receive = [0.2, 0.1, 1.9, 13, -0.5, -12, 11, -0.5, 1, -1, -5, 0.3];
            %msg = [1 1 0 0 1 1];
...
%----------------------------------------end----------------------------------------
            ...
            if error ~= 0
                ...
                if mod(frame_error_count,10) == 0
                    fprintf('frame error count = %d\n', frame_error_count);
                    if frame_error_count >= max_frame_error_count
                        fprintf('SNR & truncation type = %d, %d FINISH\n\n', EbN0dB,truncation_type);
                    end
                end
                bit_error_count = bit_error_count + error;
            end
        end
        FER = frame_error_count / frame;
        BER = bit_error_count / (frame * msg_len);

        BER_picture(truncation_type,EbN0dB) = round(BER,8);
